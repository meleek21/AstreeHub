...
namespace ASTREE_PFE.Services
{
    public class EventService : IEventService
    {
        ...
        public async Task<EventResponseDTO> CreateEventAsync(EventCreateDTO eventDto)
        {
            ...
            // No automatic notifications for birthday events here
            // Birthday notifications are handled in GenerateBirthdayEventsAsync
        ...
        public async Task GenerateBirthdayEventsAsync()
        {
            try
            {
                ...
                // Get list of employees whose birthday is today
                var todaysBirthdayEmployees = employees
                    .Where(e =>
                        e.DateOfBirth.Month == currentDate.Month
                        && e.DateOfBirth.Day == currentDate.Day
                    )
                    .ToList();

                // Process today's birthdays
                if (todaysBirthdayEmployees.Any())
                {
                    // Get all employees to receive notifications
                    var allEmployeeIds = employees.Select(e => e.Id).ToList();

                    // For each person with a birthday today
                    foreach (var birthdayEmployee in todaysBirthdayEmployees)
                    {
                        // Check if birthday event already exists
                        var birthdayDate = new DateTime(
                            currentYear,
                            birthdayEmployee.DateOfBirth.Month,
                            birthdayEmployee.DateOfBirth.Day
                        );
                        bool eventExists = await _eventRepository.ExistsForEmployeeAsync(
                            birthdayEmployee.Id,
                            birthdayDate
                        );
                        ...
                        if (!eventExists)
                        {
                            ...
                            {
                                Title = $"C'est l'anniversaire de {birthdayEmployee.FullName}!",
                                ...
                                IsRecurring = true,
                                IsOpenEvent = true,
                                AssociatedEmployeeId = birthdayEmployee.Id,
                            };
                            await CreateEventAsync(birthdayEventDto);
                            _logger.LogInformation(
                                $"Created birthday event for {birthdayEmployee.FullName} on {birthdayDate:d}"
                            );
                        }
                        else
                        {
                            _logger.LogInformation(
                                $"Birthday event already exists for {birthdayEmployee.FullName} on {birthdayDate:d}"
                            );
                        }

                        // Create and send birthday notification - this is the ONLY place we send birthday notifications
                ...
                // Process upcoming birthday events (without notifications) for planning purposes
                for (int i = 0; i < employees.Count; i += batchSize)
                {
                    ...
                    foreach (var employee in batch)
                    {
                        ...
                        // Only process future birthdays and skip already processed today's birthdays
                        if (
                            birthdayDate < currentDate
                            || (
                                birthdayDate.Month == currentDate.Month
                                && birthdayDate.Day == currentDate.Day
                            )
                        )
                            continue;
                        if (
                            await _eventRepository.ExistsForEmployeeAsync(employee.Id, birthdayDate)
                        )
                        {
                            _logger.LogInformation(
                                $"Birthday event already exists for {employee.FullName} on {birthdayDate:d}"
                            );
                            continue;
                        }
                        var birthdayEventDto = new EventCreateDTO
                        ...
                        await CreateEventAsync(birthdayEventDto);
                        _logger.LogInformation(
                            $"Created birthday event for {employee.FullName} on {birthdayDate:d}"
                        );
                    }
                }
            }
        ...
        public async Task<IEnumerable<EventResponseDTO>> GetEventsByOrganizerAsync(
            string organizerId
        )
        {
        ...
        public async Task<IEnumerable<EventResponseDTO>> GetEventsByCategoryAsync(
        ...
        )
        {
            ...
            if (@event.AttendeeStatusFinal.TryGetValue(employeeId, out bool isFinal) && isFinal)
                throw new InvalidOperationException(
                    "Cannot change attendance status after it has been finalized."
                );

            // If the status is Accepted or Declined, mark it as final
            bool markAsFinal =
                status == AttendanceStatus.Accepté || status == AttendanceStatus.Refusé;

            // Update attendance status first
            var statusUpdated = await _eventRepository.UpdateAttendanceStatusAsync(
                eventId,
                employeeId,
                status
            );
            ...
            if (statusUpdated)
            {
                ...
                // Notify organizer if status is Accepted or Declined - expanded to include declined status
                if (
                    (status == AttendanceStatus.Accepté || status == AttendanceStatus.Refusé)
                    && !string.IsNullOrEmpty(@event.Organizer)
                    && @event.Organizer != employeeId
                ) // Don't notify if user is responding to their own event
                {
                    ...
                    await _notificationService.CreateEventStatusChangeNotificationAsync(
                        eventId,
                        @event.Title,
                        @event.Organizer, // Send to organizer
                        status,
                        attendeeName // Show who responded
                    );
                }
            }
        ...
        public async Task<Dictionary<AttendanceStatus, int>> GetAttendanceStatusCountsAsync(
            string eventId
        )
        {
            var @event = await _eventRepository.GetByIdAsync(eventId);
