...
api.interceptors.response.use(
  ...
  async (error) => {
    if (error.response?.status === 401) {
      // Only clear token and dispatch authError for protected endpoints
      ...
      if (isProtected) {
        console.log(
          "Unauthorized access on protected endpoint, clearing token"
        );
...
export const postsAPI = {
  getAllPosts: (lastItemId, limit = 10) =>
    api.get(`/post?lastItemId=${lastItemId || ""}&limit=${limit}`),
  getChannelPosts: (channelId, lastItemId, limit = 10) =>
    api.get(
      `/post/channel/${channelId}?lastItemId=${lastItemId || ""}&limit=${limit}`
    ),
  ...
  createChannelPost: (channelId, postData) =>
    api.post(`/post/channel/${channelId}`, postData),
  updateChannelPost: (channelId, postId, postData) =>
    api.put(`/post/channel/${channelId}/post/${postId}`, postData),
  deleteChannelPost: (channelId, postId) =>
    api.delete(`/post/channel/${channelId}/${postId}`),
  getPostsByAuthor: (authorId, lastItemId, limit = 10) =>
    api.get(
      `/post/author/${authorId}?lastItemId=${lastItemId || ""}&limit=${limit}`
    ),
  //admin-event endpoints
  getEventPosts: (lastItemId, limit = 10) =>
    api.get(`/post/event?lastItemId=${lastItemId || ""}&limit=${limit}`),
  addEventPost: (eventPostData) =>
    api.post("/post/event/create", eventPostData),
  updateEventPost: (id, eventPostData) =>
    api.put(`/post/event/update/${id}`, eventPostData),
...
export const reactionsAPI = {
  ...
  getReactionsByEmployee: (employeeId) =>
    api.get(`/reaction/employee/${employeeId}`),
  getReactionByEmployeeAndPost: (employeeId, postId) =>
    api.get(`/reaction/employee/${employeeId}/post/${postId}`),
...
export const userAPI = {
  ...
  updateProfile: (userId, formData) =>
    api.put(`/employee/${userId}`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }),
  updateOwnProfile: (formData) =>
    api.put("/employee/me/update-profile", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }),
...
export const userOnlineStatusAPI = {
  ...
  updateUserStatus: (userId, isOnline) =>
    api.post(`/UserOnlineStatus/${userId}/status`, isOnline),
  updateLastActivity: (userId) =>
    api.post(`/UserOnlineStatus/${userId}/activity`),
};

// Channels API service
export const channelsAPI = {
  getAllChannels: () => api.get("/Channel"),
  getUserChannels: (departmentId) =>
    api.get(
      `/Channel/user${departmentId ? `?departmentId=${departmentId}` : ""}`
    ),
...
export const messagesAPI = {
  ...
  getUserConversations: (userId) =>
    api.get(`/message/users/${userId}/conversations`),
  getConversation: (conversationId, userId) =>
    api
      .get(`/message/conversations/${conversationId}?userId=${userId}`)
      .then((res) => ({ ...res.data, creatorId: res.data.creatorId })),
  ...
  markMessageAsRead: (messageId, statusDto) =>
    api.put(`/message/messages/${messageId}/read-status`, statusDto),
  deleteMessage: (messageId) => api.delete(`/message/messages/${messageId}`),
  // Message endpoints
  editMessage: (messageId, messageData) =>
    api.put(`/message/messages/${messageId}/edit`, messageData, {
      headers: {
        "Content-Type": "application/json",
      },
    }),

  unsendMessage: (messageId, userId) =>
    api.delete(`/message/messages/${messageId}/unsend`, {
      headers: {
        "Content-Type": "application/json",
      },
  ...
  softDeleteMessage: (messageId, userId) =>
    api.put(
      `/message/messages/${messageId}/soft-delete`,
  ...
  softDeleteConversation: (conversationId, userId) =>
    api.put(
      `/message/conversations/${conversationId}/soft-delete`,
      JSON.stringify(userId),
      {
        headers: {
          "Content-Type": "application/json",
  ...
  createConversation: (conversationData) =>
    api.post("/message/conversations", conversationData),

  // Unread messages
  getUnreadMessagesCount: (userId) =>
    api.get(`/message/users/${userId}/unread-messages/count`),

  // Attachment upload endpoint
  uploadMessageAttachment: (formData) =>
    api.post("/message/upload-message-attachment", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }),
  ...
  permanentlyDeleteGroup: (conversationId, userId) =>
    api.delete(`/message/conversations/${conversationId}/permanent`, {
      params: { userId },
    }),
  addParticipant: (conversationId, userId, newParticipantId) =>
    api.post(
      `/message/conversations/${conversationId}/participants?userId=${userId}&newParticipantId=${newParticipantId}`
    ),
  leaveGroup: (conversationId, userId) =>
    api.post(`/message/conversations/${conversationId}/leave?userId=${userId}`),
  getParticipants: (conversationId) =>
    api.get(`/message/conversations/${conversationId}/participants`),
};
...
export const commentsAPI = {
  ...
  addReply: (commentId, replyData) =>
    api.post(`/comment/${commentId}/reply`, replyData),
  ...
  updateReply: (commentId, replyId, content) =>
    api.put(`/comment/${commentId}/reply/${replyId}`, { content }),
...
export const eventsAPI = {
  ...
  addAttendee: (eventId, attendeeData) =>
    api.post(`/event/${eventId}/attendee`, attendeeData),
  removeAttendee: (eventId, attendeeId) =>
    api.delete(`/event/remove-attendee/${eventId}/${attendeeId}`),
  updateAttendanceStatus: (eventId, attendeeId, status) =>
    api.put(`/event/${eventId}/attendee/${attendeeId}/status`, { status }),
  getUserAttendanceStatus: (eventId, userId) =>
    api.get(`/event/${eventId}/user/${userId}/attendance-status`),
  ...
  inviteDepartment: (eventId, departmentId) =>
    api.post(`/event/invite-department/${eventId}/${departmentId}`),
  inviteMultiple: (eventId, employeeIds) =>
    api.post(`/event/${eventId}/invite-multiple`, employeeIds),
  ...
  getAttendanceStatusCounts: (eventId) =>
    api.get(`/event/${eventId}/attendance-counts`),
  getUpcomingEvents: () => api.get("/event/upcoming"),
  getEventsByOrganizer: (organizerId) =>
    api.get(`/event/by-organizer/${organizerId}`),
  getEventsByCategory: (category) => api.get(`/event/by-category/${category}`),
  getOpenEvents: () => api.get("/event/open-events"),
  getEventsByAttendee: (employeeId) =>
    api.get(`/event/by-attendee/${employeeId}`),
  updateEventStatus: (eventId, status) =>
    api.put(`/event/update-status/${eventId}`, status),
...
export const adminAPI = {
  ...
  assignEmployeeToDepartment: (employeeId, departmentId) =>
    api.patch(`/employee/assign/${employeeId}/department`, { departmentId }),
  // Delete an employee
  ...
  updateEmployee: (employeeId, formData) =>
    api.put(`/employee/${employeeId}`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }),
...
export const libraryAPI = {
  GetLibraryPosts: (lastItemId, limit = 10) =>
    api.get(`/post/library?lastItemId=${lastItemId || ""}&limit=${limit}`),
...
export const notificationAPI = {
  ...
  getUnreadNotifications: (userId) =>
    api.get(`/notification/unread?userId=${userId}`),
  ...
  markAsRead: (notificationId, userId) =>
    api.put(`/notification/${notificationId}/read?userId=${userId}`),

  // Mark all notifications as read
  markAllAsRead: (userId) =>
    api.put(`/notification/mark-all-read?userId=${userId}`),
...
export const todoAPI = {
  ...
  createTodo: (todoData, userId) =>
    api.post(`/todo/newTask?userId=${userId}`, todoData),

  // Get a specific todo by ID
  getTodoById: (id, userId) => api.get(`/todo/${id}?userId=${userId}`),

  // Get all todos with optional filtering
  getTodos: (userId, status, priority, dueDate) =>
    api.get("/todo/readAll", {
      params: { userId, status, priority, dueDate },
    }),

  // Update a todo
  updateTodo: (id, todoData, userId) =>
    api.put(`/todo/update/${id}?userId=${userId}`, todoData),
