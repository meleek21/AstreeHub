...
namespace ASTREE_PFE.Hubs
{
    ...
    public class MessageHub : Hub
    {
        ...
        private static readonly Dictionary<string, string> _userConnectionMap =
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            if (!string.IsNullOrEmpty(userId))
            {
                ...
                var userHub = Context
                    .GetHttpContext()
                    .RequestServices.GetRequiredService<IHubContext<UserHub>>();
        ...
        public async Task SendMessage(MessageCreateDto messageDto)
        {
            ...
            _logger.LogInformation(
                "User {UserId} attempting to send message in conversation {ConversationId}",
                userId,
            ...
            try
            {
                ...
                _logger.LogInformation(
                    "Calling CreateMessageAsync for User {UserId} with DTO: {@MessageDto}",
                    userId,
                    messageDto
                );
                ...
                _logger.LogInformation(
                    "CreateMessageAsync successful for User {UserId}. Message ID: {MessageId}",
                    userId,
                    message.Id
                );
                ...
                _logger.LogInformation(
                    "Message {MessageId} sent to group conversation_{ConversationId}",
                    message.Id,
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed to send message for User {UserId} in conversation {ConversationId}. DTO: {@MessageDto}",
                    userId,
                    messageDto.ConversationId,
                    messageDto
                );
        ...
        public async Task SendTypingIndicator(string conversationId)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "User {UserId} attempted to send typing indicator for conversation {ConversationId} they are not part of.",
                        userId,
                        conversationId
                    );
                    throw new HubException("User not part of this conversation");
                ...
                await Clients
                    .GroupExcept($"conversation_{conversationId}", Context.ConnectionId)
                    .SendAsync("UserTyping", userId, conversationId);
                _logger.LogInformation(
                    "User {UserId} sent typing indicator for conversation {ConversationId}",
                    userId,
                    conversationId
                );
            }
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "Failed to send typing indicator for User {UserId} in conversation {ConversationId}",
                    userId,
                    conversationId
                );
        ...
        public async Task EditMessage(string messageId, string updatedContent)
        {
            ...
            try
            {
                ...
                if (success)
                {
                    ...
                    await Clients
                        .Group($"conversation_{message.ConversationId}")
                        .SendAsync("MessageEdited", messageId, updatedContent);
                }
                else
                {
        ...
        public async Task UnsendMessage(string messageId)
        {
            ...
            try
            {
                ...
                if (success)
                {
                    ...
                    await Clients
                        .Group($"conversation_{message.ConversationId}")
                        .SendAsync("MessageUnsent", messageId);
                }
                else
                {
